...
@data_poisoner
def poison(img, label, prob_poison=0.2):
...
def get_dataset():
    ...
    poisoned_test_data = poison(test_data, prob_poison=1.0) # Poison the whole test dataset for checking the backdoor task

    assert isinstance(test_data, Dataset)
    assert isinstance(poisoned_test_data, Dataset)

    return flex_dataset, test_data, poisoned_test_data, poisoned_clients_ids
...
if __name__ == "__main__":
    flex_dataset, test_data, poisoned_test_data, poisoned_clients_ids = get_dataset()
    main()